...
case "$(echo "$EXTENSION" | tr '[:upper:]' '[:lower:]')" in
	...
	export LIBVA_DRIVER_NAME=iHD
        mpvpaper -o "--input-ipc-server=$SOCKET loop-file=inf --mute --no-osc --no-osd-bar --hwdec=vaapi --vaapi-device=/dev/dri/renderD129 --gpu-context=wayland --no-input-default-bindings" '*' "$WALL" &
	...
	rm -f "$VIDEO_CACHE"
        ...
        bash "$HOME/.config/hypr/scripts/wallpapers/update-hyprlock.sh"
